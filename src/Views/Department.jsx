...
function Department() {
  ...
  const handlesubmit = async (e) => {
    ...
    try {
      if (isEditing) {
        await dispatch(
          updatedepartmentBYid({ id: editingId, updateData: formdata })
        ).unwrap();
        setIsopen(false);
  ...
  return (
    <div className="flex relative w-full">
      ...
      <div className="flex flex-col ms-3 w-[80%] relative">
        <div>Department</div>
        ...
        {Isopen && (
          <div className="shadow-2xl rounded-md bg-black inset-0 bg-opacity-50 z-50 fixed p-20 pt-36 ">
            <form onSubmit={handlesubmit} className="bg-white p-5 rounded-2xl shadow-lg w-96 mx-auto">
              <h1 className="text-center text-xl font-bold">Department</h1>
              <div
                className="absolute top-0 -right-10 bg-black text-white cursor-pointer"
              ...
              {getdatabyrid.map((item) => {
                return (
                  <div>
                    ...
                    >
                      {" "}
                      <button className="bg-blue-700 text-white rounded-md px-7 py-2">
                        Cancel
                      </button>{" "}
                    </div>
                  </div>
